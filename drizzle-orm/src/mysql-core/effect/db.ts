...
> {
	 ...
	 * ```ts
	 * // Replace one row
	 * yield* db.replace(cars).values({ id: 1, brand: 'BMW' });
	 *
	 * // Replace multiple rows
	 * yield* db.replace(cars).values([{ id: 1, brand: 'BMW' }, { id: 2, brand: 'Porsche' }]);
	 * ```
	 */
	replace<TTable extends MySqlTable>(
		table: TTable,
	): MySqlInsertBuilder<TTable, TQueryResult, MySqlEffectInsertHKT<TEffectHKT>> {
		return new MySqlInsertBuilder(table, this.session, this.dialect, MySqlEffectInsertBase, true);
	}
